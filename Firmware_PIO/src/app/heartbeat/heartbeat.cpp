...
static void read_config(HeartbeatAppForeverData *cfg)
{
    ...
    if (size == 0)
    {
        ...
        // 默认值
        cfg->role = 0; // 角色
        strcpy(cfg->client_id, "mqttx_32673c0d");
        strcpy(cfg->subtopic, "heart/test");
        cfg->mqtt_server.fromString(DEFALUT_MQTT_IP);
        cfg->port = DEFALUT_MQTT_PORT; // mqtt服务端口
        strcpy(cfg->server_user, "root");
        strcpy(cfg->server_password, "Root_2022");
        write_config(cfg);
    }
    else
    {
