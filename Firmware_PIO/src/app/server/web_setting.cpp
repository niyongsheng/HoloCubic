...
#define WEATHER_SETTING "<form method=\"GET\" action=\"saveWeatherConf\">"                                                                                          \
                        ...
                        "<label class=\"input\"><span>禅道 Cookie zentaosid</span><input type=\"text\"name=\"custom_secret\"value=\"%s\"></label>"              \
                        "<label class=\"input\"><span>禅道 URL</span><input type=\"text\"name=\"custom_remark\"value=\"%s\"></label>"                        \
                        "</label><input class=\"btn\" type=\"submit\" name=\"submit\" value=\"保存\"></form>"
...
#define STOCK_SETTING "<form method=\"GET\" action=\"saveStockConf\">"                                                                                          \
                      "<label class=\"input\"><span>股票代码,例如：sz000001或sh000001</span><input type=\"text\"name=\"stock_id\"value=\"%s\"></label>" \
                      "<label class=\"input\"><span>数据更新周期（毫秒）</span><input type=\"text\"name=\"updataInterval\"value=\"%s\"></label>"      \
                      "</label><input class=\"btn\" type=\"submit\" name=\"submit\" value=\"保存\"></form>"
...
#if APP_WEATHER_USE
    webpage_header += F("<li><a href='/weather_setting'>时钟气象站</a></li>");
#endif
...
{
    webpage_footer = F("<footer>&copy;niyongsheng 2023</footer>");
...
void HomePage()
{
    ...
    webpage = F("<a href='https://github.com/niyongsheng/HoloCubic' target='_blank'><button>Github</button></a>");
    webpage += F("<a href='https://space.bilibili.com/241980240?spm_id_from=333.788.b_765f7570696e666f.1' target='_blank'><button>BiliBili教程</button></a>");
    Send_HTML(webpage);
}
...
void weather_setting()
{
    ...
    char custom_secret[128];
    char custom_remark[128];
    // 读取数据
    ...
    app_controller->send_to(SERVER_APP_NAME, "Weather", APP_MESSAGE_GET_PARAM,
                            (void *)"custom_secret", custom_secret);
    app_controller->send_to(SERVER_APP_NAME, "Weather", APP_MESSAGE_GET_PARAM,
                            (void *)"custom_remark", custom_remark);
...
void saveWeatherConf(void)
{
    ...
    app_controller->send_to(SERVER_APP_NAME, "Weather",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"custom_secret",
                            (void *)server.arg("custom_secret").c_str());
    app_controller->send_to(SERVER_APP_NAME, "Weather",
                            APP_MESSAGE_SET_PARAM,
                            (void *)"custom_remark",
                            (void *)server.arg("custom_remark").c_str());
    // 持久化数据
